...
class UNetSparse(nn.Module):
    ...
    def forward(self, x: torch.Tensor, batch_size=1, valid_mask=None) -> BlockContent:
        output = self.model(BlockContent(x, valid_mask), batch_size)
        return output
...
class UNetBlockOuterSparse(UNetBlock):
    ...
    def forward(self, x: BlockContent, batch_size: int):
        ...
        if should_concat:
            ...
            if  config.GENERAL.LEVEL == "256" and proxy_output is not None:
